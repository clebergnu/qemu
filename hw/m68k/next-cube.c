...
{
    DPRINTF("SCSI IRQ NUM %p %i %i %x\n", opaque, n, level, int_mask);
...
static void serial_irq(void *opaque, int n, int level)
{
...
{
    qemu_irq *ser_irq = qemu_allocate_irqs(serial_irq, cpu, 2);
    DeviceState *dev;
    SysBusDevice *s;
